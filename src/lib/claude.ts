...
【タイトルのフォーマット — 必須】
title は「{クライアント名} × ボタリズム — {テーマ}」の形で、必ずクライアント名を先頭に置く。
- 例: 「田中商店 × ボタリズム — 自走への第一歩」
- 例: 「みおり × ボタリズム — "わかってるけどできない"の正体」
- クライアント名が会話から判別できない／"不明" の場合は「クライアント × ボタリズム — {テーマ}」とする。
...
タイトルは「{クライアント名} × ボタリズム — {テーマ}」の順（例：「田中商店 × ボタリズム — 自走への第一歩」）。`,
...
タイトルは「{クライアント名} × ボタリズム — {テーマ}」の順（例：「田中商店 × ボタリズム — "丸投げ体質"からの脱却・自走意識が芽生えた転換点」）。`,
...
タイトルは「{クライアント名} × ボタリズム — {テーマ}」の順で、深層テーマを象徴する記憶に残る表現に（例：「みおり × ボタリズム — "わかってるけどできない"の正体／自己効力感の再構築に向けた転換点」）。`,
};
...
const JSON_SCHEMA_EXTENDED = `{
  "title": "「{クライアント名} × ボタリズム — {テーマ}」の形で必ずクライアント名を先頭に置く（クライアント名が不明なら「クライアント × ボタリズム — {テーマ}」）",
