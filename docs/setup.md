...
自分のコーチング哲学・関わり方を反映した深層分析モードを使いたい場合は、以下の環境変数を設定するとモード切り替え UI に「ブランドモード」が表示されるようになります。

| 変数 | 例 | 説明 |
|---|---|---|
| `NEXT_PUBLIC_BRAND_MODE_ENABLED` | `true` | UI でモード切り替えを表示するフラグ |
| `NEXT_PUBLIC_BRAND_LABEL` | `塚越メンタリング` | UI ボタン上のラベル |
| `BRAND_NAME` | `塚越アカデミー` | サービス名 |
| `BRAND_SHORT_NAME` | `塚越` | タイトルに使う短縮名（例: 「田中商店 × 塚越 — テーマ」） |
| `BRAND_HOST_NAME` | `塚越さん` | 提供者の通称（話者A の自動検出にも使う） |
| `BRAND_HOST_FULL_NAME` | `塚越暁` | フルネーム |
| `BRAND_HOST_KEYWORDS` | `塚越,塚越さん` | 話者検出用のキーワード（カンマ区切り） |
| `BRAND_PHILOSOPHY` | `（自分の哲学を1〜2文で）` | システムプロンプトに埋め込まれる |
| `BRAND_APPROACH` | `（自分のアプローチを1〜2文で）` | 同上 |
| `BRAND_HOST_STRENGTH` | `（自分の強み・特徴を1〜2文で）` | 同上 |
| `BRAND_SESSION_FLOW` | `課題の明確化 → 仮説 → 検証` | 典型的なセッションの流れ |

これらが未設定なら **汎用議事録モードのみ** が動作し、塚越さん／第三者向けにはこちらがおすすめです。
