...
## ステップ 8. （任意）ブランドモード（自分のトンマナ）の設定

EchoNote はデフォルトで **PlaudNote 風の汎用議事録モード** で動きます（誰のセッションでも違和感なく使えるトーン）。

自分のコーチング哲学・関わり方を反映した「深層分析モード」を有効化したい場合は、**EchoNote の管理画面から設定** できます（再デプロイ不要・即時反映）。

### 手順

1. EchoNote のトップページ右上の **「管理」** ボタンを押す（ベージュ色になり「管理中」表示）
2. その横に出る **「ブランド設定」** ボタンを押す
3. パネルで以下を入力:

| 項目 | 例 | 説明 |
|---|---|---|
| ブランドモードを有効化 | チェック | オフのままだと汎用モードのみ動作 |
| ブランド名 | `塚越アカデミー` | サービス名 |
| 短縮ブランド名 | `塚越` | タイトルに使う（例: 「田中商店 × 塚越 — テーマ」） |
| ホスト名（通称） | `塚越さん` | 話者A の自動検出にも使う |
| ホストのフルネーム | `塚越暁` | |
| 話者検出キーワード | `塚越,塚越さん` | カンマ区切り |
| UIモードラベル | `塚越メンタリング` | UI ボタン上のラベル |
| サービス哲学 | （1〜2文で） | システムプロンプトに埋め込まれる |
| アプローチ | （1〜2文で） | 同上 |
| ホストの強み | （1〜2文で） | 同上 |
| セッションの流れ | `課題の明確化 → 仮説 → 検証` | 典型的なセッションの流れ |

4. 「保存」を押す → DB に保存され、再デプロイなしで即反映される

### ブランドモードを使わない場合

何も設定しなければ、汎用議事録モード（PlaudNote 風）のみが動作します。第三者向けの議事録としてはむしろこちらが無難です。

### env による bootstrap 設定（任意）

開発時の初期値として env から読み込むこともできます（`.env.example` 参照）。env に設定値があると初回起動時にフォームが prefill されますが、本格運用では管理画面で保存して DB 管理に切り替えるのを推奨します。
